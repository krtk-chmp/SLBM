...
def store_index_fo(con: sqlite3.Connection, d: date, text: str) -> int:
    """Index futures + options aggregates (NIFTY, BANKNIFTY, ...) from the F&O bhavcopy."""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    fut: dict[str, tuple] = {}
    opt: dict[str, list[int]] = {}
    for r in csv.DictReader(io.StringIO(text)):
        if "FinInstrmTp" in r:  # UDIFF (2024 onwards)
            t = r.get("FinInstrmTp")
            sym = (r.get("TckrSymb") or "").strip()
            if t == "IDF":
                exp = r.get("XpryDt", "")
                if sym not in fut or exp < fut[sym][0]:
                    fut[sym] = (exp, _f(r.get("ClsPric")), _f(r.get("UndrlygPric")),
                                _i(r.get("OpnIntrst")))
            elif t == "IDO":
                side = (r.get("OptnTp") or "").strip()
                if side in ("CE", "PE"):
                    a = opt.setdefault(sym, [0, 0])
                    a[0 if side == "CE" else 1] += _i(r.get("OpnIntrst"))
        else:  # old format (pre Jul-2024)
            ...
            if inst == "FUTIDX":
                try:
                    ...
                    continue
                if sym not in fut or exp < fut[sym][0]:
                    fut[sym] = (exp, _f(r.get("CLOSE")), 0.0, _i(r.get("OPEN_INT")))
            elif inst == "OPTIDX":
                side = (r.get("OPTION_TYP") or "").strip()
                if side in ("CE", "PE"):
                    a = opt.setdefault(sym, [0, 0])
                    a[0 if side == "CE" else 1] += _i(r.get("OPEN_INT"))
    rows = []
    for sym, (exp, fc, sp, foi) in fut.items():
        ce, pe = opt.get(sym, (0, 0))
        rows.append((d.isoformat(), sym, exp, fc, sp or None, foi, ce, pe))
    con.executemany("INSERT OR REPLACE INTO index_fo VALUES (?,?,?,?,?,?,?,?)", rows)
    con.commit()
    return len(rows)


def store_index_members(con: sqlite3.Connection, lists: dict) -> int:
    n = 0
    for name, syms in lists.items():
        con.execute("DELETE FROM index_members WHERE index_name=?", (name,))
        con.executemany("INSERT OR REPLACE INTO index_members VALUES (?,?)",
