...
class NSEClient:
    ...
    def index_constituents(self) -> dict[str, list[str]]:
        """Official index membership lists from niftyindices.com."""
        out = {}
        sess = cr.Session(impersonate="chrome")  # own session: NSE cookies break this host
        for name, fn in self.INDEX_LISTS.items():
            try:
                r = sess.get(f"https://www.niftyindices.com/IndexConstituent/{fn}", timeout=30)
                if r.status_code != 200 or b"Symbol" not in r.content[:400]:
                    log.warning("index list %s unavailable (HTTP %s)", name, r.status_code)
                    continue
                ...
                log.warning("index list %s failed: %s", name, e)
        return out
