...
def fetch_day(nse: NSEClient, con, d: date, skip_existing=True) -> bool:
    ...
    if fo:
        ...
        ingest.store_index_fo(con, d, fo)
...
def main():
    ...
    # index membership (Nifty 50, Bank Nifty, ...) — changes a couple of times a year
    lists = nse.index_constituents()
    if lists:
        log.info("index membership refreshed: %d entries across %d indexes",
                 ingest.store_index_members(con, lists), len(lists))
    else:
        # fall back to the checked-in seed list so the filter still works
        n = con.execute("SELECT COUNT(*) FROM index_members").fetchone()[0]
        if not n:
            seed = Path(__file__).parent / "data" / "index_members_seed.csv"
            if seed.exists():
                import csv as _csv
                rows = list(_csv.DictReader(seed.open()))
                con.executemany("INSERT OR REPLACE INTO index_members VALUES (?,?)",
                                [(r["index_name"], r["symbol"]) for r in rows])
                con.commit()
                log.info("index membership seeded from file (%d rows)", len(rows))
        log.warning("index membership fetch unavailable this run")
