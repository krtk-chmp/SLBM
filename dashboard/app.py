...
@st.cache_data(ttl=3600)
def index_members() -> dict:
    out = {}
    try:
        rows = get_con().execute("SELECT index_name, symbol FROM index_members").fetchall()
    except Exception:
        return out  # table not created yet (first run of the new pipeline)
    for name, sym in rows:
        out.setdefault(name, set()).add(sym)
    return out


@st.cache_data(ttl=3600)
def index_data() -> pd.DataFrame:
    try:
        return pd.read_sql_query("SELECT * FROM index_fo ORDER BY date", get_con())
    except Exception:
...
def render_indexes():
    df = index_data()
    if df.empty:
        st.info("Index data is still being collected — it appears after the nightly "
                "update runs. Check back tomorrow morning.")
        return
    st.caption(f"Index data up to **{df['date'].max()}**. Indexes cannot be lent — "
               "this is market context, not a lending screen.")
    ...
    for sym in ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "NIFTYNXT50"]:
        ...
        if d.empty:
            continue
        r = d.iloc[-1]
        spot = r["spot_close"] if pd.notna(r["spot_close"]) and r["spot_close"] else None
        fut = r["fut_close"]
        level = spot or fut or 0
        pcr = (r["pe_oi"] / r["ce_oi"]) if r["ce_oi"] else None
        prev = d.iloc[-6] if len(d) > 6 else None
        pcr_prev = (prev["pe_oi"] / prev["ce_oi"]) if prev is not None and prev["ce_oi"] else None

        if pcr is None:
            mood, cls, right = "no options data", "quiet", ""
        elif pcr >= 1.2:
            mood, cls = "put-heavy — traders are hedged / cautious", "hot"
            right = f'<div class="pct-hot">PCR {pcr:.2f}</div>'
        elif pcr <= 0.8:
            mood, cls = "call-heavy — traders leaning optimistic", "warm"
            right = f'<div class="pct-warm">PCR {pcr:.2f}</div>'
        else:
            mood, cls = "balanced positioning", "quiet"
            right = f'<div class="pct-quiet">PCR {pcr:.2f}</div>'

        st.markdown(
            f'<div class="row"><div><div class="sym">{INDEX_LABELS.get(sym, sym)}</div>'
            f'<div class="sub">{mood}</div></div>'
            f'<div class="nums"><div class="fee">{level:,.0f}</div>{right}</div></div>',
            unsafe_allow_html=True)

        with st.expander("Details"):
            days = max((pd.to_datetime(r["expiry"]) - pd.to_datetime(r["date"])).days, 1)
            coc = ((fut - spot) / spot * (365 / days) * 100) if spot and fut else None
            kv = ""
            if spot:
                kv += f'<div><span>Index level</span>{spot:,.0f}</div>'
            if fut:
                kv += f'<div><span>Futures</span>{fut:,.0f}</div>'
            kv += f'<div><span>Futures open positions</span>{int(r["fut_oi"]):,}</div>'
            if pcr is not None:
                kv += f'<div><span>Put-call ratio</span>{pcr:.2f}</div>'
            if coc is not None:
                kv += f'<div><span>Carry</span>{coc:+.1f}% / yr</div>'
            st.markdown(f'<div class="kv">{kv}</div>', unsafe_allow_html=True)

            if pcr is not None and pcr_prev is not None:
                move = "rising" if pcr > pcr_prev else "falling"
                st.markdown(f"Put-call ratio is **{move}** this week "
                            f"({pcr_prev:.2f} → {pcr:.2f}).")

            hist = d.copy()
            hist["date"] = pd.to_datetime(hist["date"])
            hist["PCR"] = hist["pe_oi"] / hist["ce_oi"].replace(0, pd.NA)
            h2 = hist.dropna(subset=["PCR"]).set_index("date")[["PCR"]].tail(180)
            if len(h2) > 5:
                st.markdown("**Put-call ratio trend**")
                st.line_chart(h2, height=200)
            st.caption("Above 1.0 = more put positions than calls (cautious). Below 1.0 = more calls. "
                       "This describes how traders are positioned — our 5-year test found it does "
                       "not predict which way the market goes.")
...
def render_shares():
    ...
    choices = ["All SLBM stocks"] + [n for n in ["NIFTY 50", "NIFTY BANK", "NIFTY NEXT 50", "NIFTY 500"]
                                     if n in members]
    pick_idx = st.selectbox("Show stocks from", choices, key="idx_filter")
    market = scores[~scores["symbol"].isin(set(ordered))].sort_values("score", ascending=False)
    if pick_idx != "All SLBM stocks":
        market = market[market["symbol"].isin(members.get(pick_idx, set()))]
        if market.empty:
            st.caption(f"No {pick_idx} stocks are trading in SLBM today.")
...
mode = st.radio("View", ["Shares — lending", "Indexes — market view"],
                horizontal=True, label_visibility="collapsed")
if mode.startswith("Shares"):
    render_shares()
else:
    render_indexes()
